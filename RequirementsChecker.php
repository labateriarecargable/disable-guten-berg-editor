...
class RequirementsChecker
{
    ...
    public function pluginVersionNotice($title, $version)
    {
        echo '<div class="error">';
        echo "<p>The &#8220;" . esc_html(
                $this->title
            ) . "&#8221; plugin cannot run on " . $title . " versions older than " . $version . '. Please update ' . $title . '.</p>';
        echo '</div>';
    }
    ...
    protected function pluginsActive()
    {
        foreach ($this->plugins as $title => $file) {
            $plugin_version = get_file_data(ABSPATH . 'wp-content/plugins/'.$file['file'], [
                'Version' => 'Version',
            ], 'plugin');
            $plugin_version = $plugin_version['Version'];
            $required_version = $file['version'];
            if (! in_array($file['file'], apply_filters('active_plugins', get_option('active_plugins')))) {
                update_option($this->getPluginNotActiveOptionName(), $title, false);
                add_action('admin_notices', [$this, 'pluginNotActiveNotice']);
                return false;
            }else if(self::isVersionAtLeast($plugin_version, $required_version)){
                return true;
            }else{
                add_action('admin_notices', 
                function() use ( $title, $required_version) { 
                self::pluginVersionNotice( $title, $required_version); });
                return false;
            }
